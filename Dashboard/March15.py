...
app.layout = html.Div([html.H1("Tests"), dash_table.DataTable(df.to_dict('records'), style_cell={'textAlign':'left', 'whiteSpace': 'normal','height':'auto'}, \
                                  columns=[{"name": i, "id": i} for i in df.columns], style_header={'fontWeight' :'bold', 'fontSize':18}, \
                                 style_cell_conditional=[{'if' : {'column_id':'Has the test been tested? (Y/N)'},'width':'10%'}, \
                                                         {'if' : {'column_id':'Current Result (P/F/X)'}, '        width':'10%'}, 
                                                         
               { 'if': {
                'filter_query': '{Test run?} contains "F"' },
